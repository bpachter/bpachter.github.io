...
function stretchFromSlider(v: number): number {
  return Math.pow(10, (v / 1000) * 3.05);
}
function sliderFromStretch(s: number): number {
  return clamp(Math.round((Math.log10(s) / 3.05) * 1000), 0, 1000);
}
function bandName(s: number): string {
  if (s < 1.05) return 'visible — barely shifted';
  if (s < 1.8) return 'visible, reddened';
  if (s < 50) return 'infrared — JWST territory';
  if (s < 1000) return 'far infrared';
  return 'sub-mm — the CMB’s neighborhood';
}

/* ───────────────────────── 11 · ripples, stretched ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-pond');
  if (!cv) return;
  const fig = cv.closest('.fig') as HTMLElement;
  const slider = $<HTMLInputElement>('#pond-z');
  ...
  $$('.pond-chip', fig).forEach(b => b.addEventListener('click', () => {
    st.auto = false;
    st.target = Number((b as HTMLElement).dataset.s);
  }));

  function visRamp(u: number): string {
  ...
  function draw(t: number): void {
    ...
    if (st.auto && !reduced) {
      st.S *= 1 + dt * 0.28;
      if (st.S >= 1120) { st.S = 1120; st.auto = false; }
    }
    if (!isNaN(st.target)) {
      const ls = Math.log(st.S), lt = Math.log(st.target);
      const nl = ls + (lt - ls) * Math.min(1, dt * 3);
      st.S = Math.exp(nl);
      if (Math.abs(lt - nl) < 0.01) { st.S = st.target; st.target = NaN; }
    }
    const S = st.S;
    ...
    for (const p of sprite) {
      ctx.globalAlpha = 0.7;
      ctx.beginPath(); ctx.arc(56 + p[0] * 22, midY + p[1] * 13, 1, 0, TAU); ctx.fill();
    }
    // you: an eye
    ctx.globalAlpha = 0.9; ctx.strokeStyle = TEAL; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.ellipse(W - 46, midY, 17, 10, 0, 0, TAU); ctx.stroke();
    ctx.fillStyle = TEAL_B;
    ctx.beginPath(); ctx.arc(W - 46, midY, 4, 0, TAU); ctx.fill();
    ...
    for (let x = x0; x <= x1; x += 2) {
      ...
      if (sLoc < 1.8) { ctx.strokeStyle = visRamp((sLoc - 1) / 0.8); ctx.globalAlpha = 0.9; ctx.setLineDash([]); }
      else if (sLoc < 1000) { ctx.strokeStyle = FAINT; ctx.globalAlpha = 0.75; ctx.setLineDash([]); }
      else { ctx.strokeStyle = VIOLET; ctx.globalAlpha = 0.8; ctx.setLineDash([4, 4]); }
      ctx.lineWidth = 1.6;
    ...
    for (const [sThr, lab] of [[1.8, 'leaves the visible'], [1000, 'enters the microwave']] as Array<[number, string]>) {
      if (S <= sThr) continue;
      const u = Math.pow((sThr - 1) / (S - 1), 1 / 1.5);
      const x = x0 + u * (x1 - x0);
      ctx.globalAlpha = 0.6; ctx.strokeStyle = FAINT;
      ctx.beginPath(); ctx.moveTo(x, midY - 44); ctx.lineTo(x, midY + 44); ctx.stroke();
    ...
    if (document.activeElement !== slider) slider.value = String(sliderFromStretch(S));
  }
  autoOnView(fig, () => { if (!touch.touched && !reduced) st.auto = true; }, 700);
  loopWhenVisible(cv, draw);
})();

/* ───────────────────────── 12 · the instrument ladder ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-ladder');
  if (!cv) return;
  const fig = cv.closest('.fig') as HTMLElement;
  const slider = $<HTMLInputElement>('#lad-z');
  const readout = $('#lad-read');

  // log10(λ in meters): axis from 100 nm (-7) to 10 cm (-1)
  const L0 = -7, L1 = -1;
  interface Inst { name: string; lo: number; hi: number; }
  const INST: Inst[] = [
  ...
  function fmtLam(m: number): string {
    if (m < 1e-6) return fmt(m * 1e9, 0) + ' nm';
    if (m < 1e-3) return fmt(m * 1e6, m < 1e-5 ? 1 : 0) + ' μm';
    if (m < 1e-2) return fmt(m * 1e3, 1) + ' mm';
  ...
  function draw(t: number): void {
    ...
    const X = (lg: number): number => ax0 + ((lg - L0) / (L1 - L0)) * (ax1 - ax0);
    ...
    for (const [lo, hi, nm] of BANDS) {
      ...
      ctx.globalAlpha = 0.6; ctx.fillStyle = FAINT;
      ctx.fillText(nm, (X(lo) + X(hi)) / 2, axY + 26);
    }
    ctx.globalAlpha = 0.6; ctx.strokeStyle = LINE_C; ctx.lineWidth = 1;
    ...
    ctx.globalAlpha = 0.5; ctx.strokeStyle = GOLD; ctx.setLineDash([2, 4]);
    ctx.beginPath(); ctx.moveTo(X(Math.log10(550e-9)), axY); ctx.lineTo(mx, axY); ctx.stroke();
    ctx.setLineDash([]);
    ctx.globalAlpha = 0.95; ctx.fillStyle = GOLD;
    ctx.shadowColor = GOLD; ctx.shadowBlur = 10;
    ctx.beginPath(); ctx.arc(mx, axY, 5, 0, TAU); ctx.fill();
    ctx.shadowBlur = 0;
    ...
    INST.forEach((ins, i) => {
      ...
      if (on) {
        ctx.globalAlpha = 0.25; ctx.strokeStyle = TEAL;
        ctx.beginPath(); ctx.moveTo(mx, y); ctx.lineTo(mx, axY - 8); ctx.stroke();
      }
    });
  ...
  autoOnView(fig, () => { if (!touch.touched && !reduced) st.auto = true; }, 900);
  loopWhenVisible(cv, draw);
})();

/* ───────────────────────── 13 · the bubble in 3-D ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-bubble');
  if (!cv) return;
  ...
  const cam: Cam = { yaw: 0.5, pitch: 0.16, cx: 0, cy: 0, scale: 100, dist: 4.4 };
  ...
  function draw(t: number): void {
    ...
    const dt = last ? Math.min(0.05, (t - last) / 1000) : 0.016;
    last = t;
    if (!orb.everDragged && !reduced) cam.yaw += dt * 0.06;
    cam.cx = W / 2; cam.cy = H / 2; cam.scale = Math.min(W, H) * 0.40;
    ctx.clearRect(0, 0, W, H);
    ...
    for (const p of inner) {
      ...
      ctx.fillStyle = SKY;
      ctx.beginPath(); ctx.arc(q[0], q[1], 0.7 + 1.2 * dn, 0, TAU); ctx.fill();
    }
    // the wall
    for (let i = 0; i < N; i++) {
      ...
      ctx.beginPath(); ctx.arc(q[0], q[1], 1 + 1.7 * dn, 0, TAU); ctx.fill();
    }
    // someone else's bubble spills past our wall
    if (showOther) {
      const oc = project(cam, OG);
      ctx.globalAlpha = 0.9; ctx.fillStyle = TEAL_B;
      ctx.beginPath(); ctx.arc(oc[0], oc[1], 2.6, 0, TAU); ctx.fill();
      for (const p of other) {
        const q = project(cam, p);
        const dn = clamp((q[2] - 0.68) * 2.0, 0, 1);
        ctx.globalAlpha = 0.08 + 0.22 * dn;
        ctx.fillStyle = TEAL_B;
        ctx.beginPath(); ctx.arc(q[0], q[1], 1, 0, TAU); ctx.fill();
      }
      ctx.globalAlpha = 0.85; ctx.fillStyle = TEAL_B;
    ...
    // you + radius
    ctx.globalAlpha = 0.95; ctx.strokeStyle = TEAL; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.arc(cam.cx, cam.cy, 5, 0, TAU); ctx.stroke();
    const rp = project(cam, [0.985, -0.1, 0.12]);
    ctx.globalAlpha = 0.55; ctx.strokeStyle = FAINT; ctx.setLineDash([3, 5]);
    ctx.beginPath(); ctx.moveTo(cam.cx, cam.cy); ctx.lineTo(rp[0], rp[1]); ctx.stroke();
    ctx.setLineDash([]);
    ctx.globalAlpha = 0.85; ctx.fillStyle = FAINT;
...
(function () {
  const cv = $<HTMLCanvasElement>('#w-band');
  if (!cv) return;
  const fig = cv.closest('.fig') as HTMLElement;
  ...
  const a = (t: number): number => Math.pow(t / T, 2 / 3);
  // comoving seat of the source, chosen so the photon lands on you exactly at T
  const CHI0 = 3 * T * (1 - Math.pow(t0 / T, 1 / 3));   // ≈ 40 Gly
  const chiPhoton = (t: number): number => CHI0 - 3 * T * (Math.pow(t / T, 1 / 3) - Math.pow(t0 / T, 1 / 3));

  const st = { p: 0, playing: false, done: false };
  const touch = userTouch(fig);
  function syncPlay(): void { playBtn.textContent = st.playing ? 'pause' : (st.done ? 'replay' : 'play'); playBtn.classList.toggle('on', st.playing); }
  playBtn.addEventListener('click', () => {
    if (st.done) { st.p = 0; st.done = false; }
  ...
  let last = 0;
  function draw(tm: number): void {
    const [ctx, W, H] = fit(cv);
    const dt = last ? Math.min(0.05, (tm - last) / 1000) : 0.016;
    last = tm;
    if (st.playing && !reduced) {
      st.p += dt / 17;
      if (st.p >= 1) { st.p = 1; st.playing = false; st.done = true; syncPlay(); }
    }
    ...
    ctx.beginPath(); ctx.moveTo(youX - CHI0 * at * K, bandY); ctx.lineTo(youX, bandY); ctx.stroke();
    ctx.font = '10.5px "JetBrains Mono", monospace'; ctx.textAlign = 'center';
    for (let m = 5; m < CHI0; m += 5) {
      const x = youX - m * at * K;
      ctx.globalAlpha = 0.45; ctx.strokeStyle = LINE_C;
      ctx.beginPath(); ctx.moveTo(x, bandY - 5); ctx.lineTo(x, bandY + 5); ctx.stroke();
      ctx.globalAlpha = 0.55; ctx.fillStyle = FAINT;
    ...
    for (const p of sprite) {
      ...
      ctx.beginPath(); ctx.arc(srcX + p[0] * 15, bandY - 40 + p[1] * 9, 1, 0, TAU); ctx.fill();
    }
    ctx.globalAlpha = 0.8; ctx.fillStyle = FAINT;
    ctx.fillText('the source', srcX, bandY - 62);
    // you
    ctx.globalAlpha = 0.95; ctx.strokeStyle = TEAL; ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.arc(youX, bandY - 40, 6, 0, TAU); ctx.stroke();
    ctx.globalAlpha = 0.8; ctx.fillStyle = FAINT;
    ...
    ctx.beginPath(); ctx.arc(phX, bandY - 40, 4, 0, TAU); ctx.fill();
    ctx.shadowBlur = 0;
    ...
    if (st.done) {
      ctx.globalAlpha = 0.9; ctx.fillStyle = TEAL_B; ctx.textAlign = 'center';
      ctx.fillText('13.8-billion-year-old light, from a source now ~40 Gly away — with the full expansion history, 46.5', W / 2, H - 16);
    }
    ctx.globalAlpha = 1;
  }
  autoOnView(fig, () => { if (!touch.touched && !reduced && !st.playing) playBtn.dispatchEvent(new Event('click')); }, 1100);
  loopWhenVisible(cv, draw);
})();
