...
/* ───────────────────────── 01 · the pull-back ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-scale');
  if (!cv) return;
  const fig = cv.closest('.fig') as HTMLElement;
  const slider = $<HTMLInputElement>('#scale-z');
  ...
  const nearStars: Array<[number, number, number]> = [];
  for (let i = 0; i < 90; i++) nearStars.push([gauss(rng) * 11, gauss(rng) * 11, 0.5 + rng()]);

  // The galaxy, offset: its center sits ~26 kly away from you.
  const GC: [number, number] = [-20000, 16000];
  const mwArm: Array<[number, number]> = [];
  for (let i = 0; i < 1300; i++) {
    const r = 50000 * Math.sqrt(rng());
    const th = (r / 50000) * 4.6 + (i % 2) * Math.PI + gauss(rng) * 0.24;
    mwArm.push([r * Math.cos(th), r * Math.sin(th)]);
  }
  const mwBulge: Array<[number, number]> = [];
  for (let i = 0; i < 220; i++) mwBulge.push([gauss(rng) * 7000, gauss(rng) * 5600]);

  const dwarfs: Array<[number, number]> = [];
  for (let i = 0; i < 26; i++) dwarfs.push([gauss(rng) * 1.1e6, gauss(rng) * 1.1e6]);
  const m31: Array<[number, number]> = [];
  for (let i = 0; i < 150; i++) m31.push([1.9e6 + gauss(rng) * 1.0e5, -1.6e6 + gauss(rng) * 6.0e4]);
  const m33: Array<[number, number]> = [];
  for (let i = 0; i < 80; i++) m33.push([2.35e6 + gauss(rng) * 5.0e4, -0.95e6 + gauss(rng) * 3.4e4]);

  // Web inside the underdensity (sparse) and the denser shell beyond the ring.
  const webIn: Array<[number, number]> = [];
  {
    const cs: Array<[number, number]> = [];
    for (let i = 0; i < 15; i++) cs.push([(rng() * 2 - 1) * 0.8, (rng() * 2 - 1) * 0.8]);
    for (let i = 0; i < 430; i++) {
      ...
      for (let k = 0; k < 5; k++) {
        ...
        if (d < R) { x += ((x - cs[bi][0]) / d) * (R - d) * 0.5; y += ((y - cs[bi][1]) / d) * (R - d) * 0.5; }
      }
      webIn.push([x * 0.95e9, y * 0.95e9]);
    }
  }
  const webOut: Array<[number, number]> = [];
  {
  ...
  const st = { z: LOG_MIN, mode: 'auto' as 'auto' | 'idle', target: NaN };
  const touch = userTouch(fig, () => { st.mode = 'idle'; st.target = NaN; });

  function fovLabel(fov: number): string {
    if (fov < 0.55) return fmt(fov * 63241, 1) + ' AU';
    if (fov < 1e3) return fmt(fov, fov < 20 ? 1 : 0) + ' light-years';
    if (fov < 1e6) return fmt(fov / 1e3, 0) + ' thousand ly';
    if (fov < 1e9) return fmt(fov / 1e6, fov < 2e7 ? 1 : 0) + ' million ly';
    return fmt(fov / 1e9, 2) + ' billion ly';
  }
  function label(ctx: Ctx, s: string, x: number, y: number, color: string, a: number): void {
    ctx.globalAlpha = a; ctx.fillStyle = color;
    ctx.font = '10.5px "JetBrains Mono", monospace';
    ctx.textAlign = 'center'; ctx.fillText(s, x, y); ctx.globalAlpha = 1;
  }
  /** alpha window: fade a layer in above lo, out above hi (z in log-ly). */
  function win(z: number, lo: number, hi: number): number {
  ...
  function draw(t: number): void {
    ...
    if (st.mode === 'auto' && !reduced) {
      ...
      if (st.z >= LOG_MAX) { st.z = LOG_MAX; st.mode = 'idle'; }
    }
    if (!isNaN(st.target)) {
      st.z += (st.target - st.z) * Math.min(1, dt * 3);
      if (Math.abs(st.target - st.z) < 0.01) { st.z = st.target; st.target = NaN; }
    }
    ...
    const orbits: Array<[number, string]> = [[1.58e-5, 'Earth'], [8.2e-5, 'Jupiter'], [4.74e-4, 'Neptune']];
    for (let oi = 0; oi < orbits.length; oi++) {
      const r = orbits[oi][0], nm = orbits[oi][1];
      const rp = r * s;
      if (rp < 2 || rp > W * 2.5) continue;
      ctx.globalAlpha = 0.5; ctx.strokeStyle = FAINT; ctx.lineWidth = 1;
      ctx.beginPath(); ctx.arc(cx, cy, rp, 0, TAU); ctx.stroke();
      const ang = 0.9 + oi * 1.9;
      ctx.globalAlpha = 0.9; ctx.fillStyle = SKY;
      ctx.beginPath(); ctx.arc(cx + rp * Math.cos(ang), cy + rp * Math.sin(ang), 2, 0, TAU); ctx.fill();
      if (rp > 40 && rp < W * 0.52) label(ctx, nm, cx, cy - rp - 6, FAINT, 0.8);
    }
    // Sun at center while we're at solar/stellar scales
    const aSun = win(st.z, -5.2, 1.9);
    if (aSun > 0) {
      ctx.globalAlpha = aSun; ctx.fillStyle = GOLD;
      ctx.beginPath(); ctx.arc(cx, cy, Math.max(1.6, clamp(4.6e-7 * s, 1.6, 9)), 0, TAU); ctx.fill();
    }
    // Oort cloud
    const rOort = 1.6 * s;
    if (rOort > 8 && rOort < W * 2) {
      ctx.globalAlpha = 0.35; ctx.strokeStyle = FAINT; ctx.setLineDash([2, 5]); ctx.lineWidth = 1;
      ctx.beginPath(); ctx.arc(cx, cy, rOort, 0, TAU); ctx.stroke(); ctx.setLineDash([]);
      if (rOort < W * 0.5) label(ctx, 'Oort cloud', cx, cy - rOort - 6, FAINT, 0.6);
    }
    // near stars
    ...
    if (aNs > 0) {
      ...
      for (const p of nearStars) {
        ...
        ctx.beginPath(); ctx.arc(cx + p[0] * s, cy + p[1] * s, 1.2, 0, TAU); ctx.fill();
      }
    }
    ...
    if (aMw > 0) {
      ...
      ctx.rotate(-0.5); ctx.scale(1, 0.56);
      ctx.fillStyle = STAR_C;
      for (const p of mwArm) {
        ctx.globalAlpha = aMw * 0.32;
        ctx.beginPath(); ctx.arc(p[0] * s, p[1] * s, 1, 0, TAU); ctx.fill();
      }
      ctx.fillStyle = GOLD;
    ...
    if (aLg > 0) {
      ...
      for (const p of dwarfs) {
        ...
        ctx.beginPath(); ctx.arc(cx + p[0] * s, cy + p[1] * s, 1.1, 0, TAU); ctx.fill();
      }
      ctx.fillStyle = TEAL_B;
      for (const p of m31) {
        ctx.globalAlpha = aLg * 0.3;
        ctx.beginPath(); ctx.arc(cx + p[0] * s, cy + p[1] * s, 1, 0, TAU); ctx.fill();
      }
      for (const p of m33) {
        ctx.globalAlpha = aLg * 0.24;
        ctx.beginPath(); ctx.arc(cx + p[0] * s, cy + p[1] * s, 0.9, 0, TAU); ctx.fill();
      }
    ...
    if (aWeb > 0) {
      ...
      const rk = 1.0e9 * s;
      if (rk > 60) {
        ctx.globalAlpha = aWeb * 0.55; ctx.strokeStyle = VIOLET; ctx.setLineDash([6, 7]); ctx.lineWidth = 1.2;
        ctx.beginPath(); ctx.arc(cx, cy, rk, 0, TAU); ctx.stroke(); ctx.setLineDash([]);
    ...
    ctx.globalAlpha = 0.95; ctx.strokeStyle = TEAL; ctx.lineWidth = 1.4;
    ctx.beginPath(); ctx.arc(cx, cy, 6, 0, TAU); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(cx - 11, cy); ctx.lineTo(cx - 3, cy); ctx.moveTo(cx + 3, cy); ctx.lineTo(cx + 11, cy);
    ctx.moveTo(cx, cy - 11); ctx.lineTo(cx, cy - 3); ctx.moveTo(cx, cy + 3); ctx.lineTo(cx, cy + 11); ctx.stroke();
  ...
  slider.addEventListener('input', () => {
    st.mode = 'idle'; st.target = NaN;
    st.z = LOG_MIN + (Number(slider.value) / 1000) * (LOG_MAX - LOG_MIN);
  });
  $$('.z-chip', fig).forEach(b => b.addEventListener('click', () => {
    st.mode = 'idle';
    st.target = Number((b as HTMLElement).dataset.z);
  }));
  if (reduced) { st.mode = 'idle'; st.z = LOG_MAX; }
  autoOnView(fig, () => { if (!touch.touched && !reduced) st.mode = 'auto'; }, 500);
  loopWhenVisible(cv, draw);
})();

/* ───────────────────────── 02 · the web, in 3-D ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-web');
  if (!cv) return;
  ...
  for (let i = 0; i < 820; i++) {
    ...
    for (let k = 0; k < 6; k++) {
      ...
      if (d < R) {
        const f = (R - d) * 0.55 / d;
        x += (x - voids[bi][0]) * f; y += (y - voids[bi][1]) * f; z += (z - voids[bi][2]) * f;
      }
    }
  ...
  const cam: Cam = { yaw: 0.7, pitch: 0.32, cx: 0, cy: 0, scale: 100, dist: 3.6 };
  ...
  function draw(t: number): void {
    ...
    const dt = last ? Math.min(0.05, (t - last) / 1000) : 0.016;
    last = t;
    if (!orb.everDragged && !reduced) cam.yaw += dt * 0.1;
    cam.cx = W / 2; cam.cy = H / 2; cam.scale = Math.min(W, H) * 0.62;
    ctx.clearRect(0, 0, W, H);
    ...
    if (showVoids) {
      ...
      for (const v of voids) {
        ...
        ctx.beginPath(); ctx.arc(p[0], p[1], v[3] * cam.scale * p[2], 0, TAU); ctx.stroke();
      }
      ctx.setLineDash([]);
    }
    for (let i = 0; i < pts.length; i++) {
      ...
      ctx.beginPath(); ctx.arc(p[0], p[1], 0.7 + 1.6 * dn, 0, TAU); ctx.fill();
    }
    // the Attractor knot
    {
      ...
      ctx.beginPath(); ctx.arc(p[0], p[1], 3.4 + 2 * p[2], 0, TAU); ctx.fill();
      ctx.shadowBlur = 0;
    ...
    // you (and the Local Group), alone in a void
    {
      ...
      for (const q of you) {
        ...
        ctx.globalAlpha = 0.95;
        ctx.beginPath(); ctx.arc(p[0], p[1], 2, 0, TAU); ctx.fill();
      }
      ctx.globalAlpha = 0.9; ctx.fillStyle = TEAL_B;
      ctx.font = '10.5px "JetBrains Mono", monospace'; ctx.textAlign = 'left';
...
/* ───────────────────────── 03 · the tug-of-war ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-tug');
  if (!cv) return;
  const fig = cv.closest('.fig') as HTMLElement;
  const sepEl = $<HTMLInputElement>('#tug-sep');
  const massEl = $<HTMLInputElement>('#tug-mass');
  ...
  const A = 21.4;
  const rStar = (m: number): number => Math.cbrt(A * m);

  const rng = mulberry32(5);
  const sprite: Array<[number, number]> = [];
  for (let i = 0; i < 46; i++) {
  ...
  type Mode = 'idle' | 'run' | 'merged' | 'gone';
  const st = { mode: 'idle' as Mode, d: 2.5, v: 0, burst: 0 };
  const touch = userTouch(fig);

  function params(): [number, number] { return [Number(sepEl.value), Number(massEl.value)]; }
  function syncLabels(): void {
    const [d0, m] = params();
    sepVal.textContent = fmt(d0, 1) + ' Mly';
    massVal.textContent = fmt(m, 1) + ' galaxies';
    if (st.mode === 'idle') st.d = d0;
    const rs = rStar(m);
  ...
  function reset(): void { st.mode = 'idle'; st.v = 0; st.burst = 0; syncLabels(); }
  sepEl.addEventListener('input', reset);
  massEl.addEventListener('input', reset);
  goBtn.addEventListener('click', () => {
    if (st.mode === 'run') return;
    const [d0] = params();
    st.d = d0; st.v = 0; st.burst = 0; st.mode = 'run';
  });
  reBtn.addEventListener('click', reset);

  function galaxy(ctx: Ctx, x: number, y: number, sc: number, spin: number, tint: string): void {
    ctx.save(); ctx.translate(x, y); ctx.rotate(spin); ctx.scale(1, 0.62);
    ctx.fillStyle = tint;
    for (const p of sprite) {
      ctx.globalAlpha = 0.68;
      ctx.beginPath(); ctx.arc(p[0] * sc, p[1] * sc, 1.1, 0, TAU); ctx.fill();
    }
    ctx.globalAlpha = 0.9; ctx.fillStyle = STAR_C;
  ...
  function draw(t: number): void {
    ...
    if (st.mode === 'run') {
      ...
      st.v += a * dt * 0.9;
      st.d += st.v * dt * 0.9;
      if (st.d <= 0.5) { st.d = 0.5; st.mode = 'merged'; }
      if (st.d >= 13.2) { st.d = 13.2; st.mode = 'gone'; }
    }
    if (st.mode === 'merged' && st.burst < 1) st.burst = Math.min(1, st.burst + dt * 1.4);
    ...
    ctx.beginPath(); ctx.moveTo(50, midY + 56); ctx.lineTo(W - 50, midY + 56); ctx.stroke();
    for (let mm = 0; mm <= 13; mm += 2) {
      const x = W / 2 - (mm * ppm) / 2, x2 = W / 2 + (mm * ppm) / 2;
      ctx.beginPath(); ctx.moveTo(x, midY + 52); ctx.lineTo(x, midY + 60); ctx.stroke();
      ctx.beginPath(); ctx.moveTo(x2, midY + 52); ctx.lineTo(x2, midY + 60); ctx.stroke();
    }
    ctx.setLineDash([4, 5]); ctx.globalAlpha = 0.65; ctx.strokeStyle = FAINT;
    const bx1 = W / 2 - (rs * ppm) / 2, bx2 = W / 2 + (rs * ppm) / 2;
    ctx.beginPath(); ctx.moveTo(bx1, midY - 58); ctx.lineTo(bx1, midY + 56); ctx.stroke();
    ctx.beginPath(); ctx.moveTo(bx2, midY - 58); ctx.lineTo(bx2, midY + 56); ctx.stroke();
    ctx.setLineDash([]);
    ...
    if (st.mode === 'merged') {
      ...
      if (st.burst < 1) {
        ...
        ctx.strokeStyle = STAR_C; ctx.lineWidth = 2;
        ctx.beginPath(); ctx.arc(W / 2, midY, 10 + st.burst * 90, 0, TAU); ctx.stroke();
      }
      ctx.globalAlpha = 0.9; ctx.fillStyle = FAINT; ctx.textAlign = 'center';
      ctx.fillText('merged — one galaxy now (this is Andromeda and us, eventually)', W / 2, midY + 84);
    } else {
      const grav = A * m / (st.d * st.d), dark = st.d;
      const mx = Math.max(grav, dark);
      // force meters
      ctx.textAlign = 'left'; ctx.font = '10.5px "JetBrains Mono", monospace';
      ctx.globalAlpha = 0.9; ctx.fillStyle = TEAL;
      ctx.fillRect(56, 22, (grav / mx) * 120, 5);
      ctx.fillStyle = VIOLET;
      ctx.fillRect(56, 40, (dark / mx) * 120, 5);
      ctx.fillStyle = FAINT;
      ctx.fillText('gravity', 56, 16); ctx.fillText('dark energy', 56, 58);
      // galaxies + arrows
      ...
      ctx.globalAlpha = 0.85; ctx.fillStyle = FAINT; ctx.textAlign = 'center';
  ...
  syncLabels();
  autoOnView(fig, () => { if (!touch.touched && !reduced) goBtn.dispatchEvent(new Event('click')); }, 1400);
  loopWhenVisible(cv, draw);
})();
