...
/* ───────────────────────── 08 · the watershed ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-lani');
  if (!cv) return;
  ...
  function basin(x0: number, y0: number): number {
    let x = x0, y = y0;
    for (let i = 0; i < 300; i++) {
      const [vx, vy] = vel(x, y);
      x += vx * 0.11; y += vy * 0.11;
      if (Math.hypot(x - PP.x, y - PP.y) < 0.05) return 1;
      if (Math.hypot(x - GA.x, y - GA.y) < 0.055 || x > 1.18) return 0;
      if (x < -0.15 || y < -0.15 || y > 1.15) break;
    }
  ...
  interface Flow { x: number; y: number; px: number; py: number; b: number; }
  const flows: Flow[] = [];
  function spawn(g?: Flow): Flow {
    const n: Flow = g || { x: 0, y: 0, px: 0, py: 0, b: 0 };
    n.x = rng() * 1.1 - 0.05; n.y = rng() * 1.05 - 0.02;
    n.px = n.x; n.py = n.y;
    const gi = clamp(Math.floor(n.x * GX), 0, GX - 1), gj = clamp(Math.floor(n.y * GY), 0, GY - 1);
    n.b = grid[gj * GX + gi];
    return n;
  }
  for (let i = 0; i < 230; i++) flows.push(spawn());

  // Click-to-drop probes: bright, persistent trails (keep the last three).
  interface Probe { pts: number[]; x: number; y: number; done: boolean; }
  const probes: Probe[] = [];
  ...
  autoOnView(cv.closest('.fig') as HTMLElement, () => {
    if (!showDivide) dbtn.dispatchEvent(new Event('click'));
  }, 2600);
  ...
  function draw(t: number): void {
    ...
    for (const g of flows) {
      ...
      g.px = g.x; g.py = g.y;
      if (!reduced) { g.x += vx * dt * 6.0; g.y += vy * dt * 6.0; }
      if (Math.hypot(g.x - GA.x, g.y - GA.y) < 0.035 || Math.hypot(g.x - PP.x, g.y - PP.y) < 0.032 ||
          g.x > 1.2 || g.x < -0.12 || g.y < -0.1 || g.y > 1.12) { spawn(g); continue; }
      ctx.globalAlpha = 0.5;
    ...
    for (const p of probes) {
      if (!p.done && !reduced) {
        for (let k = 0; k < 3; k++) {
          ...
          p.x += vx * dt * 2.0; p.y += vy * dt * 2.0;
          p.pts.push(p.x, p.y);
          if (Math.hypot(p.x - GA.x, p.y - GA.y) < 0.035 || Math.hypot(p.x - PP.x, p.y - PP.y) < 0.032 ||
              p.x > 1.2 || p.x < -0.12) { p.done = true; break; }
        }
      }
      ctx.globalAlpha = 0.9; ctx.strokeStyle = STAR_C; ctx.lineWidth = 1.6;
      ctx.beginPath();
      ctx.moveTo(p.pts[0] * W, p.pts[1] * H);
      for (let k = 2; k < p.pts.length; k += 2) ctx.lineTo(p.pts[k] * W, p.pts[k + 1] * H);
      ctx.stroke();
      ctx.fillStyle = STAR_C;
      ctx.beginPath(); ctx.arc(p.x * W, p.y * H, 2.4, 0, TAU); ctx.fill();
    }
    // the divide
    if (showDivide) {
      ctx.fillStyle = STAR_C;
      for (const d of divide) {
        ctx.globalAlpha = 0.4;
        ctx.beginPath(); ctx.arc(d[0] * W, d[1] * H, 1.1, 0, TAU); ctx.fill();
      }
      ctx.globalAlpha = 0.85; ctx.fillStyle = STAR_C;
    ...
    ctx.globalAlpha = 0.8; ctx.fillStyle = SKY;
    ctx.beginPath(); ctx.arc(PP.x * W, PP.y * H, 4, 0, TAU); ctx.fill();
    ctx.textAlign = 'left';
    ctx.fillText('Perseus–Pisces — the next basin over', PP.x * W + 10, PP.y * H + 4);
    ctx.globalAlpha = 0.65; ctx.fillStyle = GOLD;
    ctx.textAlign = 'right'; ctx.fillText('Shapley, beyond →', W - 10, SH.y * H + 42);
    ctx.globalAlpha = 0.65; ctx.fillStyle = VIOLET;
    ctx.textAlign = 'left'; ctx.fillText('← the Dipole Repeller', 10, RP.y * H - 26);
    ...
    ctx.beginPath(); ctx.arc(ux * W, uy * H, 5, 0, TAU); ctx.stroke();
    ctx.fillStyle = TEAL_B; ctx.textAlign = 'left';
    ctx.fillText('you — near the edge, not the middle', ux * W + 11, uy * H + 16);
    // basin watermark
    ctx.globalAlpha = 0.14; ctx.fillStyle = TEAL_B;
    ctx.font = '600 22px "Fraunces", Georgia, serif'; ctx.textAlign = 'center';
...
(function () {
  ...
  function panel(ctx: Ctx, s: Seat[], keep: number, cx: number, cy: number, R: number, t: number, ring: string): number {
    ctx.globalAlpha = 0.5; ctx.strokeStyle = ring; ctx.setLineDash([5, 6]); ctx.lineWidth = 1.2;
    ...
    for (let i = 0; i < s.length; i++) {
      ...
      ctx.beginPath(); ctx.arc(x, y, stays ? 2 : 1.6, 0, TAU); ctx.fill();
      if (stays || a > 0.2) shown++;
    }
    return shown;
  }
  ...
  function draw(t: number): void {
    ...
    const dt = last ? Math.min(0.05, (t - last) / 1000) : 0.016;
    last = t;
    if (!touch.touched) st.t += (st.target - st.t) * Math.min(1, dt * 1.6);
    ...
    ctx.globalAlpha = 0.95; ctx.strokeStyle = TEAL; ctx.lineWidth = 1.3;
    ctx.beginPath(); ctx.arc(yx, yy, 4, 0, TAU); ctx.stroke();
    ...
    if (document.activeElement !== slider) slider.value = String(Math.round(st.t * 1000));
    ctx.globalAlpha = 1;
  }
  autoOnView(fig, () => { if (!touch.touched) st.target = 1; }, 1200);
  loopWhenVisible(cv, draw);
})();

/* ───────────────────────── 10 · the suburb ───────────────────────── */
(function () {
  const cv = $<HTMLCanvasElement>('#w-suburb');
  if (!cv) return;
  const slider = $<HTMLInputElement>('#sb-crowd');
  const readout = $('#sb-read');
  const rng = mulberry32(29);

  interface Town { x: number; y: number; r: number; ph: number; sp: number; }
  function town(n: number, seed: number): Town[] {
    const rr = mulberry32(seed), out: Town[] = [];
    for (let i = 0; i < n; i++)
  ...
  interface Flash { x: number; y: number; age: number; side: number; }
  const flashes: Flash[] = [];
  const counts = [0, 0];
  const st = { crowd: 0.6 };
  ...
  function draw(t: number): void {
    ...
    for (const [side, gals, nUse] of [[0, core, nCore], [1, burb, burb.length]] as Array<[number, Town[], number]>) {
    ...
    for (let i = flashes.length - 1; i >= 0; i--) {
      ...
      ctx.lineWidth = 1.6;
      ctx.beginPath(); ctx.arc(f.x, f.y, 4 + f.age * 26, 0, TAU); ctx.stroke();
    }
    ctx.globalAlpha = 0.9; ctx.fillStyle = FAINT;
    ctx.font = '10.5px "JetBrains Mono", monospace';
    ctx.textAlign = 'left';
