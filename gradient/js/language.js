...
(function () {
    ...
    function nearest(word, k) {
        ...
        return Object.entries(words)
            .filter(([w]) => w !== word)
            .map(([w, [x, y]]) => [w, Math.hypot(x - wx, y - wy)])
            .sort((a, b) => a[1] - b[1])
            .slice(0, k);
    }
    function drawNN(word) {
        if (anim !== null) {
            cancelAnimationFrame(anim);
            anim = null;
        }
    ...
    $('#btn-emb-clear').addEventListener('click', () => { if (anim !== null)
        cancelAnimationFrame(anim); base(); $('#emb-read').innerHTML = RESET_MSG; });
    base();
