...
(function () {
    ...
    const hashS = (s) => { let h = 0; for (const c of s)
        h = (h * 31 + c.charCodeAt(0)) | 0; return Math.abs(h); };
    ...
    function classify(text) {
        ...
        for (const [name, tool] of Object.entries(TOOLS)) {
            ...
            for (const k of tool.kw) {
                ...
                else if (tokens.some(w => w.startsWith(k)))
                    s++; /* whole-word (stemmed) */
            }
            if (s > score) {
                score = s;
                best = name;
    ...
    if (inp)
        inp.addEventListener('keydown', e => { if (e.key === 'Enter') {
            e.preventDefault();
            void route(inp.value);
        } });
    $('#btn-tool-route').addEventListener('click', () => { if (inp)
        void route(inp.value); });
    const touch = userTouch($('#w-tools'));
    autoOnView($('#w-tools'), () => { if (!touch.touched)
        void route('what\'s the weather in Raleigh?'); }, 1000);
})();
