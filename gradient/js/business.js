...
(function () {
    ...
    cv.addEventListener('mouseleave', () => { if (hover !== -1) {
        hover = -1;
        showDetail(-1);
        if (reduced || !onScreen)
            draw();
    } });
    cv.addEventListener('click', e => { const h = hitTest(e); hover = h; showDetail(h); if (reduced || !onScreen)
        draw(); });
    draw();
})();
