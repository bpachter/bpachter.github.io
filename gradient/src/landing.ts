...
(function () {
  ...
  const TEMP = (): number => { const el = $<HTMLInputElement>('#in-pipe-temp'); return el ? +el.value : 0.2; };
  let picks: number[] = STEPS.map(() => 0);
  function scaledProbs(cands: Array<[string, number]>, T: number): number[] {
    const s = cands.map(([, p]) => Math.pow(p, 1 / Math.max(0.05, T)));
    const z = s.reduce((a, b) => a + b, 0) || 1;
    return s.map(v => v / z);
  }
  function sampleStep(cands: Array<[string, number]>, T: number): number {
    const sp = scaledProbs(cands, T);
    let r = rngP();
    for (let i = 0; i < sp.length; i++) { r -= sp[i]; if (r <= 0) return i; }
    return sp.length - 1;
  }
  function resample(): void { picks = STEPS.map(s => sampleStep(s.cands, TEMP())); }
  ...
  const tempSlider = $<HTMLInputElement>('#in-pipe-temp');
  if (tempSlider) tempSlider.addEventListener('input', () => {
    $('#o-pipe-temp').textContent = (+tempSlider.value).toFixed(2);
    resample();
    if (reduced) draw(STEPS.length - 1, 1, true);
  });

  if (reduced) { draw(STEPS.length - 1, 1, true); return; }   /* static: the clean, canonical (greedy) answer */

  let t0: number | null = null, onScreen = false, curPass = -1;
  function loop(ts: number): void {
    if (t0 === null) t0 = ts;
    const total = STEPS.length * CYCLE + HOLD;
    const elapsed = ts - t0;
    const passN = Math.floor(elapsed / total);
    if (passN !== curPass) { curPass = passN; resample(); }   /* fresh sample each full pass */
