...
(function () {
  ...
  let running = false, greenOnce = false, mode: 'look' | 'blind' = 'look';
  ...
  async function run(script: Round[]): Promise<void> {
    ...
    ($('#btn-loop-break') as HTMLButtonElement).disabled = !greenOnce;
  }
  ...
  $$('#seg-loop button').forEach(b => b.addEventListener('click', () => {
    ...
    mode = (b as HTMLElement).dataset.mode as 'look' | 'blind';   /* selection is instant; feels alive mid-run */
    if (running) return;                                          /* the in-flight run finishes; new mode applies on the next */
    log.innerHTML = ''; iterEl.textContent = '0'; term.innerHTML = '$ waiting…';
    greenOnce = false; ($('#btn-loop-break') as HTMLButtonElement).disabled = true;
  }));
...
(function () {
  ...
  let solo: Side, crew: Side, simT = 0, running = false, crewN = 4;
  const rejectSet = new Set([2, 6, 9]);          /* these tasks bounce once */
  const crewCount = () => { const el = $<HTMLInputElement>('#in-crew'); return el ? +el.value : 4; };
  ...
  function draw(): void {
    ...
    for (const [s, workers, verify] of [[solo, 1, false], [crew, crewN, true]] as Array<[Side, number, boolean]>) {
  ...
  const crewEl = $<HTMLInputElement>('#in-crew');
  if (crewEl) crewEl.addEventListener('input', () => { $('#o-crew').textContent = crewEl.value; reset(); });
})();
