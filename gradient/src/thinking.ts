...
const sleepT = (ms: number) => new Promise<void>(res => window.setTimeout(res, reduced ? Math.min(ms / 4, 120) : ms));
...
(function () {
  ...
  function tick(el: Element, ok: boolean, label: string): void {
...
/* ══════════════ 18 · GRPO ══════════════ */
(function () {
  const stratEl = $('#rlvr-strat'); if (!stratEl) return;
  const rollsEl = $('#rlvr-rolls');
  interface Strat { name: string; p: number; color: string; }
  const STRATS: Strat[] = [
  ...
  let w = [0.25, 0.25, 0.25, 0.25], round = 0, auto: number | null = null;
  const rng = mulberry32(777);
  const bars: HTMLElement[] = [], pcts: HTMLElement[] = [];

  STRATS.forEach(s => {
    const row = document.createElement('div'); row.className = 'strat-row';
    ...
    const bar = document.createElement('div'); bar.className = 'bar'; bar.style.background = s.color;
  ...
  function doRound(): void {
    round++;
    rollsEl.innerHTML = '';
    const picks: number[] = [], rewards: number[] = [];
    for (let i = 0; i < 8; i++) {
      const s = sampleStrat();
      const win = rng() < STRATS[s].p;
      picks.push(s); rewards.push(win ? 1 : 0);
    ...
    const z = w.reduce((a, b) => a + b, 0); w = w.map(v => v / z);
    $('#rlvr-mean').textContent = Math.round(mean * 100) + '%';
    render();
  }
  function stopAuto(): void { if (auto !== null) { clearInterval(auto); auto = null; $('#btn-rlvr-auto').textContent = 'auto-train'; } }
  $('#btn-rlvr-round').addEventListener('click', () => { stopAuto(); doRound(); });
  $('#btn-rlvr-auto').addEventListener('click', () => {
    if (auto !== null) { stopAuto(); return; }
    $('#btn-rlvr-auto').textContent = 'stop';
    let n = 0;
    auto = window.setInterval(() => { doRound(); if (++n >= 12) stopAuto(); }, reduced ? 2100 : 1150);
  });
  ...
  visible(stratEl, v => { if (!v) stopAuto(); });
  render();
  const touch = userTouch($('#w-rlvr'));
  autoOnView($('#w-rlvr'), () => {
    if (touch.touched || auto !== null) return;
    $('#btn-rlvr-auto').textContent = 'stop';
    let n = 0;
    auto = window.setInterval(() => { doRound(); if (++n >= 12) stopAuto(); }, reduced ? 2100 : 1150);
  }, 1000);
...
(function () {
  const cv = $<HTMLCanvasElement>('#cv-budget'); if (!cv) return;
  const [ctx, W, H] = fit(cv);
  const KS = [0, 1, 2, 3, 4, 5, 6];                    /* budget = 256·2^k */
  const budget = (k: number) => 256 * Math.pow(2, k);
  const CHAIN = 640;                                    /* tokens per sampled chain */
  const serial = (b: number) => 0.34 + 0.42 * (1 - Math.exp(-b / 2200));
  ...
  function voteAcc(n: number): number {
    if (n <= 1) return pSample;
    const rng = mulberry32(9000 + n);
    let wins = 0; const TRIALS = 400;
    for (let t = 0; t < TRIALS; t++) {
      let correct = 0, trap = 0; const stray = [0, 0, 0, 0, 0];
      for (let i = 0; i < n; i++) {
        const r = rng();
        if (r < pSample) correct++;
        else if (r < pSample + (1 - pSample) * 0.42) trap++;   /* the 10¢ of this problem */
        else stray[(rng() * 5) | 0]++;
      }
      const top = Math.max(trap, ...stray);
      if (correct > top) wins++;
      else if (correct === top) wins += 0.5;
    }
    return wins / TRIALS;
  }
  const bestAcc = (n: number) => Math.min(0.965, (1 - Math.pow(1 - pSample, Math.max(1, n))) * 0.92);
  function nFor(b: number): number { return Math.max(1, Math.floor(b / CHAIN)); }

  const X = (k: number) => 70 + (k / 6) * (W - 120);
  const Y = (a: number) => H - 60 - (a - 0.3) / 0.7 * (H - 120);

  function curves(): Array<{ name: string; color: string; f: (b: number) => number }> {
    return [
  ...
  function draw(): void {
    ...
    const k = +$<HTMLInputElement>('#in-budget').value, b = budget(k);
    ...
    /* the budget line */
    ctx.strokeStyle = 'rgba(45,42,36,0.45)'; ctx.lineWidth = 1.6; ctx.setLineDash([6, 5]);
    ctx.beginPath(); ctx.moveTo(X(k), Y(0.3)); ctx.lineTo(X(k), Y(1.0)); ctx.stroke(); ctx.setLineDash([]);
    /* curves + markers */
    curves().forEach((c, ci) => {
      const pts: Pt[] = [];
      for (let kk = 0; kk <= 6; kk += 0.25) pts.push([X(kk), Y(c.f(budget(kk)))]);
      rough(ctx, pts, c.color, 2.6, 40 + ci);
      const a = c.f(b);
      ctx.fillStyle = c.color; ctx.strokeStyle = INK; ctx.lineWidth = 2;
      ctx.beginPath(); ctx.arc(X(k), Y(a), 6.5, 0, 7); ctx.fill(); ctx.stroke();
      ctx.font = '14px Karla, sans-serif'; ctx.fillStyle = c.color;
  ...
  if (!reduced) {
    ...
    const timer = window.setInterval(() => {
      ...
      const s = $<HTMLInputElement>('#in-budget');
      let v = +s.value + dir;
      if (v >= 6) { v = 6; dir = -1; } else if (v <= 0) { v = 0; dir = 1; }
...
/* ══════════════ 20 · INSIDE THE MIND ══════════════ */
(function () {
  const cv = $<HTMLCanvasElement>('#cv-sae'); if (!cv) return;
  const [ctx, W, H] = fit(cv);
  const FEATS = [
  ...
  const THOUGHTS: Array<Record<number, number>> = [
  ...
  function steerFrac(): number { return +$<HTMLInputElement>('#in-steer').value / 100; }
  function coeffs(): number[] {
  ...
  function draw(): void {
    ...
    FEATS.forEach((f, i) => {
      ...
      ctx.strokeStyle = f.color; ctx.globalAlpha = 0.5; ctx.lineWidth = 1.8;
      ctx.beginPath(); ctx.moveTo(cx, cy); ctx.lineTo(cx + ux * R, cy - uy * R); ctx.stroke();
      ctx.globalAlpha = 1;
      ctx.font = '12px Karla, sans-serif'; ctx.fillStyle = c[i] > 0.05 ? INK : FAINT;
